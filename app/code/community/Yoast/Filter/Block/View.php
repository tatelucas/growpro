...
class Yoast_Filter_Block_View extends Mage_Catalog_Block_Product_List
{
  protected function _getProductCollection()
  {
	...
	{     
		...
		$p_title = $this->__($this->getLabel()." - ".$this->getAttribute()." Overview");
        ...
        if ($breadcrumbs = $this->getLayout()->getBlock('breadcrumbs')) 
		{
            $breadcrumbs->addCrumb('home', array(
                'label' => $this->__('Home'),
                'title' => $this->__('Naar Zoekkinderboek.nl'),
	...
	{ 
		if (!$this->getData('cms_block_html')) 
		{
			...
			$html = $this->getLayout()->createBlock('cms/block')->setBlockId('landing-'.$this->getAttribute().'-'.str_replace($ReplacementValues,"-",$this->getValue()))->toHtml();
	...
	{
		return $this->__("All Products in ".$this->getLabel());
	}
	...
	public function getValue()
	{
		...
		//INSERT value label lookup here (to take label instead of ID from URL)
